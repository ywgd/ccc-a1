...
class GridLang(object):
    ...
    def insertTwitter(self,twitter):
        if self.grid.insideGrid(twitter.loc):
            self.insertDict(twitter.lang,1)
    ...
    def insertDict(self,lang,num):
        if not self.langDict.__contains__(lang):
            self.langDict[lang]=num
        else:
            self.langDict[lang]+=num
        self.totalTweets+=num
        return True


    def merge(self,other):
        self.totalTweets+=other.totalTweets
        for key,value in other.langDict.items():
            self.insertDict(key,value)
...
def mergeData(combine_data):
    ...
    for i in range(1,len(combine_data)):
        for j in range(len(combine_data[0].gridLangList)):
            results.gridLangList[j].merge(combine_data[i].gridLangList[j])
